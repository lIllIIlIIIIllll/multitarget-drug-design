...
class DEL():
    ...
    def train(self):
        ...
        for g in range(self.config.get('num_generations')):
            ...
            properties = samples.loc[:,['SAS', 'logP', 'CA9', 'GPX4', 'LPA1']] # SAS: the smaller the better, logP: the smaller the better, score: the smaller the better
            properties['LPA1'] = -properties['LPA1'] # to be minimized
            #properties['LPA3'] = -properties['LPA3'] # to be minimized
...
#samples_random = samples_random.loc[:,fieldnames] # same order of fields as samples
                ...
                common_fieldnames = [field for field in fieldnames if field in samples_random.columns]
                samples_random = samples_random.loc[:, common_fieldnames]
...
# save
            ...
            z_tree, z_mol, mu, logvar = self.get_z()
            z_tree = z_tree.cpu().numpy()
            z_mol = z_mol.cpu().numpy()
            mu = mu.cpu().numpy()
            ...
            if g<self.config.get('num_generations'):
                ...
                self.save_z(g, z_tree, z_mol, mu, logvar, samples, prefix='traindata')
            ...
            _,dists_vec=self.crowding_distance_all_fronts(properties, Fs, p_min, p_max, n_jobs=48)
            
            print('Evolutionary operations: selection, recombination, and mutation ...')
            #pm=properties.mean().to_numpy() # properties mean
            print('z shape------', z_tree.shape, z_mol.shape)
            print(z_tree, z_mol)
            
            z_tree = self.evol_ops(z_tree, rank, dists_vec, prob_ts = self.config.get('prob_ts'), crossover_method =self.config.get('crossover'), mutation_rate = self.config.get('mutation'))
            
            z_mol = self.evol_ops(z_mol, rank, dists_vec, prob_ts = self.config.get('prob_ts'), crossover_method =self.config.get('crossover'), mutation_rate = self.config.get('mutation'))
            
            

            print('Generate new samples from modified z ...')
            print('Modified z: ', z_tree.shape, z_mol.shape)
            print(z_tree, z_mol)
            # generate new samples
            #self.sampler=Sampler(self.config, self.vocab, self.trainer.model)
            # loader for z
            # zloader = DataLoader(z, batch_size=self.config.get('batch_size'))
            # z = TensorDataset(z_tree, z_mol)
            # zloader = DataLoader(z, 1)
            z_tree_loader = DataLoader(z_tree, 1)
            z_mol_loader = DataLoader(z_mol, 1)
            ...
            if g != 0:
                z_mol_iterator = iter(z_mol_loader)
                for z_tree_batch in z_tree_loader:
                    z_mol_batch = next(z_mol_iterator)
                    generated_sample = self.sampler.sample_from_z(z_tree_batch, z_mol_batch, save_results=False, seed=None)
                ...
                z_tree = z_tree[valids] # remove invalid z
                z_mol = z_mol[valids]
            ...
            # # merge new samples with old population
                fieldnames = samples.columns.values.tolist()
                common_fieldnames = [field for field in fieldnames if field in new_samples.columns]
                #samples_random = samples_random.loc[:, common_fieldnames]
                new_samples = new_samples.loc[:,common_fieldnames] # same order of fields as samples
            ...
            if g == 0:
                combined_samples = self.dataset.data.iloc[-20500:]
            ...
            #combined_properties['LPA3'] = -combined_properties['LPA3'] # to be minimized
            combined_properties = combined_properties.to_numpy()
            # sort all samples
            rank,Fs = self.fast_nondominated_sort(combined_properties)
            dists_all,dists_vec=self.crowding_distance_all_fronts(combined_properties, Fs, p_min, p_max, n_jobs=48)
...
#            print('length={}'.format(length))
            ...
            if g in [0,math.ceil(self.config.get('num_generations')/2)-1,self.config.get('num_generations')-1]:
                ...
                if self.config.get('use_gpu'):
                    self.trainer.model = self.trainer.model.cuda()
                z_tree, z_mol, mu, logvar = self.get_z(self.new_pop, g)
                z_tree = z_tree.detach().numpy()
                z_mol = z_mol.detach().numpy()
                mu = mu.detach().numpy()
                logvar = logvar.detach().numpy()
                self.save_z(g, z_tree, z_mol, mu, logvar, self.new_pop)
    ...
    def get_z(self, data=None, g=0):
        if data is not None:
            dataset = copy.deepcopy(self.dataset)
            dataset.change_data(data, g)
        ...
        #df = dataset.data
        print('zs---g-----', df.shape[0], g)
        latent_size = int(self.config.get('latent_size'))
        z_trees = torch.zeros(df.shape[0], latent_size)
        z_mols = torch.zeros(df.shape[0], latent_size)
        mus = torch.zeros(df.shape[0], latent_size * 2)
        logvars = torch.zeros(df.shape[0], latent_size * 2)
        # loader=dataset.get_loader() # load data
        loader = MolTreeFolder(dataset.config, self.config.get('train_path'), self.vocab, self.config.get('batch_size'), num_workers=4)
        ...
        for batch in loader:
            ...
            with torch.no_grad():
                loss, kl_div, wacc, tacc, sacc, z_tree, z_mol, mu, logvar, predicted_tree, predicted_mol = self.trainer.model(batch, self.config.get('a_beta'))
            start = idx*self.config.get('batch_size')
            end = start + z_tree.shape[0]
            # print('z_tree----------', z_tree.shape)
            # print('z_mol----------', z_mol.shape)
            # print('zs--------', df.shape[0])
            #zs[start:end,:]=z
            z_trees[start:end, :] = z_tree
            z_mols[start:end, :] = z_mol
            mus[start:end, :]=mu
            logvars[start:end,:]=logvar
            idx += 1

        return z_trees, z_mols, mus, logvars


    def save_z(self, g, z_tree, z_mol, mu=None, logvar=None, samples=None, prefix='pop'):
        """
        ...
        with open(filename, 'wb') as f:
            pickle.dump((z_tree, z_mol, mu, logvar, samples), file=f)
    ...
    def evol_ops(self, z, rank, dists_vec, prob_ts=0.95, crossover_method='linear', mutation_rate=0.01):
        ...
        # selection
        N = rank.shape[0]
        selected_inds = self.tournament_selection_N(N, rank, dists_vec, prob_ts=prob_ts, k=2, n_jobs=48)
...
#        return selected_inds
    ...
    def tournament_selection_N(self, num, rank, dists_vec, prob_ts, k=2, n_jobs=48):
        """
        Select num points.
        k: scalar, number of points to be randomly selected from the population.
        """
    ...
    def get_properties(self, samples, n_jobs=48):
...
#        return dists_all, dists_vec
    ...
    def crowding_distance_all_fronts( self, P, Fs, f_min, f_max, n_jobs=48):
        """
        P: properties.
        Fs: fronts.
        f_min: min values of propreties
