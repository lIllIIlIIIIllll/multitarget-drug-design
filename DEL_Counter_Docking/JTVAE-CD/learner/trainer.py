...
class Trainer:
    ...
    def _valid_epoch(self, epoch, loader):
        ...
        samples, _ = sampler.sample(num_samples, save_results=False)
    ...
    def update_vocab(self, vocab):
        self.vocab = vocab
        self.model.update_vocab(self.vocab)
        self.model = self.model.cuda()
        ...
        for epoch in range(start_epoch, start_epoch + num_epochs):
            ...
            for batch in loader:
                ...
                try:
                    ...
                    loss, kl_div, wacc, tacc, sacc, z_tree, z_mol, mu, logvar, predicted_prop_tree, predicted_prop_mol = self.model(batch, beta)

                    if self.config.get('use_gpu'):
                        # properties = properties.cuda()
                        predicted_prop_tree = predicted_prop_tree.cuda()
                        predicted_prop_mol = predicted_prop_mol.cuda()
