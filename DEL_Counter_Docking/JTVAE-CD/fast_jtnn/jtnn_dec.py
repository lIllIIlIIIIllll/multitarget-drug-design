...
class JTNNDecoder(nn.Module):
    ...
    def update_vocab(self, new_vocab, new_embedding):
        new_embedding.weight.data.normal_(0, 1)
        old_to_new_index = {old_token : new_vocab.get_index(old_token) for old_token in self.vocab.vmap if old_token in new_vocab.vmap}
        for old_index, old_token in enumerate(self.vocab.vmap):
            if old_token in new_vocab.vmap:
                new_index = old_to_new_index[old_token]
                new_embedding.weight.data[new_index] = self.embedding.weight.data[old_index]
    ...
    def forward(self, mol_batch, x_tree_vecs):
        ...
        for t in range(max_iter):
            ...
            h_nei_len = len(cur_h_nei)
            
            #Message passing
            try:
                cur_h_nei = torch.stack(cur_h_nei, dim=0).view(-1,MAX_NB,self.hidden_size)
            except:
                remain = len(cur_h_nei)
                padlen = MAX_NB - (remain - (MAX_NB * (remain // MAX_NB)))
                cur_h_nei.extend([padding] * padlen)
                h_nei_len = len(cur_h_nei)
                cur_h_nei = torch.stack(cur_h_nei, dim=0).view(-1,MAX_NB,self.hidden_size)
            ...
            try:
                ...
                cur_o_nei.extend([padding] * MAX_NB)
                cur_o_nei = cur_o_nei[:h_nei_len]
                cur_o_nei = torch.stack(cur_o_nei, dim=0).view(-1,MAX_NB,self.hidden_size)
