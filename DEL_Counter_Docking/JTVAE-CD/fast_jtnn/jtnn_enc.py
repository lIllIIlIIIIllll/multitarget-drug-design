...
class JTNNEncoder(nn.Module):
    ...
    def update_vocab(self, new_vocab, old_vocab, new_embedding):
        new_embedding.weight.data.normal_(0, 1)
        old_to_new_index = {old_token : new_vocab.get_index(old_token) for old_token in old_vocab.vmap if old_token in new_vocab.vmap}
        for old_index, old_token in enumerate(old_vocab.vmap):
