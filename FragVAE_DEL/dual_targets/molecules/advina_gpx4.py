...
def adock(receptor_input,
        ...
        seed=None,
        cpu=1,
        lig_dir = '../GPX4_new/ligand_files/',
        out_dir = '../GPX4_new/output/',
        log_dir = '../GPX4_new/log/',
        conf_dir = '../GPX4_new/config/'):
    ...
    if not os.path.isfile(ligand):
        with subprocess.Popen("exec " + '/bin/obabel -:"' + smiles + '" -O ' + ligand + ' -h --gen3d' + ' > /dev/null 2>&1', \
...
def calculateDockingScoreGPX4(mol):
    protein_surface = './DATA/protein_files/6hn3_del.pdbqt'
