...
class DEL():
    ...
    def train(self):
        ...
        for g in range(self.config.get('num_generations')):
            ...
            properties = samples.loc[:,['SAS', 'logP', 'LPA1']] # SAS: the smaller the better, logP: the smaller the better, CA9: the smaller the better, GPX4: the smaller the better
            ...
            _,dists_vec=self.crowding_distance_all_fronts(properties, Fs, p_min, p_max, n_jobs=42)
            ...
            dists_all,dists_vec=self.crowding_distance_all_fronts(combined_properties, Fs, p_min, p_max, n_jobs=42)
...
#            print('length={}'.format(length))
    ...
    def evol_ops(self, z, rank, dists_vec, prob_ts=0.95, crossover_method='linear', mutation_rate=0.01):
        ...
        # selection
        N = rank.shape[0]
        selected_inds = self.tournament_selection_N(N, rank, dists_vec, prob_ts=prob_ts, k=2, n_jobs=42)
...
#        return selected_inds
    ...
    def tournament_selection_N(self, num, rank, dists_vec, prob_ts, k=2, n_jobs=42):
        """
        Select num points.
        k: scalar, number of points to be randomly selected from the population.
        """
    ...
    def get_properties(self, samples, n_jobs=42):
...
#        return dists_all, dists_vec
    ...
    def crowding_distance_all_fronts( self, P, Fs, f_min, f_max, n_jobs=42 ):
        """
        P: properties.
        Fs: fronts.
        f_min: min values of propreties
