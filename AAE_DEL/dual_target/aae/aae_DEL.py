...
class AAE_DEL:
    ...
    def train(self):
        ...
        for g in range(generations):
            ...
            properties = samples.loc[:, ['SAS', 'logP', 'CA9', 'LPA1']]
    ...
    def get_rank_sum(self, properties):
        ...
        N = len(properties)
        sas, logp, ca9, lpa1 = properties[:, 0], properties[:, 1], properties[:, 2], properties[:, 3]

        # Get Orders of Properties
        sas_order, logp_order, ca9_order, lpa1_order = np.argsort(sas), np.argsort(logp), np.argsort(ca9), np.argsort(lpa1)

        # Get Ranks of Properties
        sas_rank, logp_rank, ca9_rank, lpa1_rank = \
            np.empty_like(sas_order), np.empty_like(logp_order), np.empty_like(ca9_order), np.empty_like(lpa1_order)
        sas_rank[sas_order], logp_rank[logp_order], ca9_rank[ca9_order], lpa1_rank[lpa1_order] = \
            np.arange(N), np.arange(N), np.arange(N), np.arange(N)

        # Get Rank Sums
        ranks = np.array([np.sum([sas_rank[i], logp_rank[i], ca9_rank[i], lpa1_rank[i]]) for i in range(N)])
        return ranks
